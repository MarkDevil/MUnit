...
import cn.autotest.framework.Utils.GlobalProperty;
...
public class ServiceConfigLoader {
	...
	private static final String Zk_addr = "zk_addr";
	private static final String Zk_protocol = "zk_protocol";
	private static final String Zk_group = "zk_group";
	...
	private ServiceConfigItem readNode(Element node) {
		...
		for (DefaultAttribute each : attrs) {
			...
			}else if (Zk_addr.equalsIgnoreCase(attrName)){
				item.setZk_addr(afterReplacement);
			}else if (Zk_group.equalsIgnoreCase(attrName)){
				item.setZk_group(afterReplacement);
			}else if (Zk_protocol.equalsIgnoreCase(attrName)){
				item.setProtocol(afterReplacement);
			}else if (Protocol.equalsIgnoreCase(attrName)){
				item.setProtocol(afterReplacement);
			}else if (Appname.equalsIgnoreCase(attrName)){
				item.setAppname(afterReplacement);
			}
			else{
