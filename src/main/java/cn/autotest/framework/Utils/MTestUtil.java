...
import cn.autotest.framework.db.DatabaseFactory;
import cn.autotest.framework.db.SQLDb;
import cn.autotest.framework.serviceClient.ClientEngine;
import cn.autotest.framework.serviceClient.ClientEngineFactory;
...
public class MTestUtil {
	private static final Logger logger = LoggerFactory.getLogger(MTestUtil.class);
	...
	public static void compareResult(Map actualResult, Map expectedResult, String msgPrefix) {
		...
		int index = 1;
		Map actual = new TreeMap<>(actualResult);
		Map expected = new TreeMap<>(expectedResult);
